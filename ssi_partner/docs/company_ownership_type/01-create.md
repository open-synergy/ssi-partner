...
- **Config:** An active `sequence.template` for model `company_ownership_type` is
...
3. Fill in the required fields:
   - **Name** _(required)_: Enter a name that identifies this ownership type (e.g.
     "Private", "State-Owned").
...
5. Optionally fill in the **Note** tab with free-text notes about this ownership type.
6. Click **Save**.

## Post-Condition

- A new **Company Ownership Type** record is created and appears in the Ownership Types
  list.
- If **Generate Code** was used, the **Code** field now holds a value from the
  configured sequence template.
- The record becomes selectable in the **Ownership Type** field on company `res.partner`
  records.
